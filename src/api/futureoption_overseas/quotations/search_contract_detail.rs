//! 해외선물 상품기본정보 — GET /uapi/overseas-futureoption/v1/quotations/search-contract-detail
//!
//! 스펙: .agent/specs/futureoption_overseas__quotations__search_contract_detail.md
//! 모의투자 미지원. QRY_CNT에 입력한 상품코드 개수를 넣고 SRS_CD_01~SRS_CD_32로 최대 32건.
...
pub const ENDPOINT: &str = "/uapi/overseas-futureoption/v1/quotations/search-contract-detail";
pub const TR_ID: &str = "HHDFC55200000";
...
pub async fn call(client: &KisClient, req: &Request) -> Result<Response> {
    if client.is_mock() {
        bail!("해외선물 상품기본정보는 모의투자 미지원 API입니다");
    }
    if req.srs_cds.is_empty() || req.srs_cds.len() > 32 {
        bail!("srs_cds 개수는 1~32건이어야 합니다 (현재 {})", req.srs_cds.len());
    }
