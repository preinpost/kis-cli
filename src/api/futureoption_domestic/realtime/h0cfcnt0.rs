//! 상품선물 실시간체결가 — WebSocket /tryitout/H0CFCNT0
//!
//! 스펙: .agent/specs/futureoption_domestic__realtime__h0cfcnt0.md
//! 실시간 WebSocket API. 모의투자 미지원.

use anyhow::{anyhow, Result};
use serde::Deserialize;

pub const TR_ID: &str = "H0CFCNT0";
...
pub fn parse_frame(data: &str) -> Result<Response> {
    ...
    if f.len() < 50 {
        return Err(anyhow!("필드 수 부족: {} < 50", f.len()));
    }
    ...
    Ok(Response {
        ...
        dscs_bltr_acml_qty: g(46),
        dynm_mxpr: g(47),
        dynm_llam: g(48),
        dynm_prc_limt_yn: g(49),
    })
}
