//! KRX야간옵션 실시간체결가 — WebSocket /tryitout/H0EUCNT0
//!
//! 스펙: .agent/specs/futureoption_domestic__realtime__h0eucnt0.md
//! 실시간 WebSocket API. 모의투자 미지원.

use anyhow::{anyhow, Result};
use serde::Deserialize;

pub const TR_ID: &str = "H0EUCNT0";
...
pub fn parse_frame(data: &str) -> Result<Response> {
    ...
    if f.len() < 56 {
        return Err(anyhow!("필드 수 부족: {} < 56", f.len()));
    }
