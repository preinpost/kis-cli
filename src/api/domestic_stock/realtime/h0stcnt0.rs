//! 국내주식 실시간체결가 (KRX) — WebSocket /tryitout/H0STCNT0
//!
//! 스펙: .agent/specs/domestic_stock__realtime__h0stcnt0.md
//! 실시간 WebSocket API. REST의 `call()` 대신 `subscribe_payload` + `parse_frame` 제공.
...
/// WebSocket 구독 메시지 생성 (등록: tr_type="1", 해제: "2")
...
/// 실시간 프레임(`^` 구분 문자열) 파싱. frame은 `|` 헤더 이후의 데이터 페이로드.
