//! 국내주식 시간외 실시간예상체결 (KRX) — WebSocket /tryitout/H0STOAC0
//!
//! 스펙: .agent/specs/domestic_stock__realtime__h0stoac0.md
//! 실시간 WebSocket API. 모의투자 미지원.

use anyhow::{anyhow, Result};
use serde::Deserialize;

pub const TR_ID: &str = "H0STOAC0";
...
pub fn parse_frame(data: &str) -> Result<Response> {
    ...
    if f.len() < 43 {
        return Err(anyhow!("필드 수 부족: {} < 43", f.len()));
    }
    Ok(Response {
