...
```bash
cargo install --git https://github.com/preinpost/kis-cli kis-cli --locked   # 워크스페이스라 패키지명(kis-cli) 지정
...
cd kis-cli
cargo run -p kis-cli --release -- install   # cargo install --path crates/kis-cli + 스킬 배포 (`--force` 덮어쓰기)
```
...
차트 뷰어(`--chart`)는 기본 feature `chart` (wry/tao)로 빌드된다. macOS 는 WKWebView 내장, Linux 는 `libwebkit2gtk-4.1-dev`, Windows 는 WebView2 런타임 필요.
헤드리스/서버(컨테이너 등)에서는 `cargo build -p kis-cli --no-default-features` 로 차트 뷰어를 빼면 GTK/WebKit 시스템 라이브러리 없이 빌드된다. 이 빌드에서 `--chart` 는 런타임에 안내 메시지와 함께 거부된다.
...
상주 데몬(시황 브리프 / 데이트레이드 / 자동손절)을 VPS 등에서 띄우는 용도. 차트 뷰어를 뺀 헤드리스 이미지라 GTK/WebKit 의존 없이 가볍다.

데몬은 **파일 하나당 하나**다 — 같이 띄울 일이 없어 `brief.yaml` / `daytrade.yaml` / `stop-loss.yaml` 로 분리했다. 셋은 같은 이미지(`kis-cli:latest`)를 빌드해 쓰고, 모든 상태(토큰 캐시·`symbols.db`·`daytrade.db`·`daytrade.toml`·`brief-stream.toml`)는 **external 볼륨 `kis-data`(`/data`)** 로 공유한다. → 토큰 1회 발급(KIS 발급 1분 1회 제한 회피)·`symbols sync` 1회면 충분.
...
docker compose -f brief.yaml build

# 4) 종목 마스터 1회 동기화 (볼륨에 symbols.db 생성)
docker compose -f brief.yaml run --rm brief symbols sync

# 5) 일회성 CLI — 아무 데몬 파일로나 run (조회/주문 등)
docker compose -f brief.yaml run --rm brief stock dome price 005930
docker compose -f brief.yaml run --rm brief auth          # 자격증명 확인
```
...
| 변수 | 의미 |
|------|------|
| `KIS_APP_KEY` / `KIS_APP_SECRET` | KIS OpenAPI 자격증명 (필수) |
| `KIS_ACCOUNT_NUMBER` | 계좌번호 `8자리-2자리` (필수) |
...
```bash
# 시황 브리프 관심종목 1회 시드 → 이후 데몬으로 상주 (국내·미국 주식 모두 지원)
docker compose -f brief.yaml run --rm brief brief stream 005930 000660 TSLA
...
docker compose -f brief.yaml run --rm brief symbols sync   # 최초 1회 (이미지도 자동 pull)
docker compose -f brief.yaml up -d                    # 또는 -f daytrade.yaml / -f stop-loss.yaml
```
...
## 아키텍처 — 카고 워크스페이스 (5 크레이트)

단일 바이너리였던 코드를 역할별 크레이트로 분리했다. `kis-core` · `kis-data` ·
`kis-analysis` · `kis-daemon` 은 **서로 의존하지 않는 leaf 라이브러리**이고, 실행 파일
`kis-cli`(바이너리명 `kis`) 만 이 넷을 가져다 쓴다. 덕분에 라이브러리들은 병렬 컴파일·
독립 테스트가 되고, SDK(`kis-core`)는 헤드리스로 어디서든 재사용할 수 있다.

```
crates/
├── kis-core/             # KIS Open API SDK (헤드리스, leaf)
│   ├── api/              #   338개 endpoint 바인딩 (OAuth/국내·해외주식/채권/선물옵션)
...
├── kis-data/             # 종목 마스터: 다운로드·파싱·SQLite FTS5 저장/검색 (leaf)
├── kis-analysis/         # 기술적 지표 순수함수 (MA/EMA/RSI/MACD/BB/Ichimoku/OBV/ATR) (leaf)
├── kis-daemon/           # 데몬 공통 인프라 (leaf, KisClient 비의존)
│   ├── logging.rs        #   tracing subscriber (단일 파일 appender + stderr / foreground)
│   ├── shutdown.rs       #   SIGTERM·Ctrl-C → graceful 종료 (CancellationToken)
│   └── config_watch.rs   #   notify 기반 설정 핫리로드
└── kis-cli/              # 실행 파일 `kis` — 위 4개 크레이트에 의존
    ├── commands/         #   CLI 서브커맨드 핸들러
    │   ├── stock/ · fo/ · daytrade/ · bond.rs · auth.rs · symbols.rs · analyze.rs
    │   ├── backtest.rs   #     7개 전략 + 스윕 + 차트 뷰어
    │   ├── signal_watch.rs · stop_loss.rs · brief.rs   # ← 데몬 (kis-daemon 인프라 사용)
    │   └── installer.rs · skill.rs
    ├── viewer/           #   wry 기반 네이티브 차트 창 (chart feature, IPC로 KIS 재호출)
    └── main.rs           #   clap 디스패치 + 크레이트 재수출(`pub use kis_core::api …`)
```

**의존 방향**: `kis-cli → { kis-core, kis-data, kis-analysis, kis-daemon }`. 라이브러리
4개는 서로 독립.

**데몬 배포**: `stop-loss` / `signal-watch` / `daytrade` / `brief` 은 각각 **별도
프로세스**(= docker-compose 서비스)로 돌고, 공통 스캐폴딩(로깅·종료·핫리로드)만
`kis-daemon` 에서 공유한다. 단일 슈퍼바이저 프로세스는 두지 않는다 — 컨테이너
오케스트레이터(compose `restart` + `tini`)가 데몬별 감시·재시작을 맡는다.

### 빌드 / 개발

```bash
cargo build -p kis-cli                       # 실행 파일 빌드 (기본: 차트 뷰어 포함)
cargo build -p kis-cli --no-default-features  # 헤드리스 (wry/tao 제외 — 컨테이너/서버)
cargo test --workspace                        # 전체 크레이트 테스트 (lib 단위테스트 포함)
cargo run  -p kis-cli -- <subcommand>         # 워크스페이스 루트에서 실행
```
