//! 해외옵션 체결추이(일간) — GET /uapi/overseas-futureoption/v1/quotations/opt-daily-ccnl
//!
//! 스펙: .agent/specs/futureoption_overseas__quotations__opt_daily_ccnl.md
//! 모의투자 미지원. 다음조회 불가. QRY_CNT + 1 개가 조회됨.
...
pub const ENDPOINT: &str = "/uapi/overseas-futureoption/v1/quotations/opt-daily-ccnl";
pub const TR_ID: &str = "HHDFO55020100";
...
pub async fn call(client: &KisClient, req: &Request) -> Result<Response> {
    if client.is_mock() {
        bail!("해외옵션 체결추이(일간)는 모의투자 미지원 API입니다");
    }
    let params = [
