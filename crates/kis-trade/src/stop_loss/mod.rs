...
use kis_core::api::domestic_stock::order_account::{inquire_balance as dome_bal, order_cash};
use kis_core::api::overseas_stock::order_account::{inquire_balance as usa_bal, order as usa_order};
use kis_core::client::KisClient;
use crate::common::resolve::format_number;
use kis_core::config;
use kis_core::ws::aes_cbc_decrypt;
...
/// 자동 손절 엔진. 로깅 초기화·시그널 리스너는 호출자(데몬 bin / kis-cli wrapper)가 소유하고
/// `cancel` 로 그레이스풀 종료를 주입한다(컨테이너 `docker compose stop` 시 SIGKILL 방지).
pub async fn run(client: &KisClient, cfg: Config, cancel: CancellationToken) -> Result<()> {
    print_banner(&cfg);
    if cfg.use_ws {
