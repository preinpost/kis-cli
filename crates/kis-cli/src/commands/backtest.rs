...
use crate::analysis::{bollinger, ichimoku, sma};
use crate::client::KisClient;
use crate::commands::analyze;
use crate::commands::helpers::{format_number, resolve_symbol};
use crate::symbols::ResolveMode;

// 전략 식별·신호 생성 계층은 kis-analysis::signals 로 이동(backtest·signal-watch·daytrade 공용).
// 여기서는 backtest 가 쓰던 경로(`crate::commands::backtest::{...}`)를 유지하기 위해 재노출만 한다.
pub use kis_analysis::signals::{
    build_strategy, compute_signals, latest_signal, Params, Series, Strategy, StrategyKind,
};
